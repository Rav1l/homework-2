...
class BearerRequestBuilder<T>: URLSessionRequestBuilder<T> {
    @discardableResult
    override func execute(_ apiResponseQueue: DispatchQueue = RickAndMortyAPI.apiResponseQueue, _ completion: @escaping (Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
...
class BearerTokenHandler {
    ...
    private static func startRefreshingToken(completionHandler: @escaping () -> Void) {
        ...
        RickAndMortyAPI.customHeaders["Authorization"] = "Bearer \(dummyBearerToken)"
